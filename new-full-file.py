...
# Discovery tuning
DISCOVERY_TOP_K = int(os.getenv("DISCOVERY_TOP_K", "20"))  # max candidates when q is provided
...
def _agent_upsert(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    doc = dict(payload)
    doc["id"] = str(agent_id)
    doc["agent_id"] = str(agent_id)

    # optional: keep appid if supplied; default same as agent_id
    doc.setdefault("appid", str(agent_id))
...
def _agents_list_full() -> List[Dict[str, Any]]:
    """
    Full list for no-q discovery mode.
    """
    container = _cosmos_container()
...
def _search_projection_fields() -> str:
    """
    Keep query payload smaller for q-search mode.
    """
    return """
    c.id,
    c.agent_id,
...
def _query_candidates_staged(q: str, top_k: int) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
    """
    Fast staged retrieval for q-search:
      1) exact match
      2) prefix match
      3) contains fallback
    """
...
# GET /api/registry/discover?q=...&debug=true|false&top=20
# Behavior:
# - if q provided: best match + justification (+ optional ranked candidates with debug=true)
# - if no q: return full agent list
...
def registry_discover(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if top_param and top_param.isdigit():
            top_k = max(1, min(int(top_param), 100))

        # CASE 1: no query => full agents
        if not q:
            agents = _agents_list_full()
        ...
        # CASE 2: q provided => ranked best match
        q_start = time.perf_counter()
        candidates, diagnostics = _query_candidates_staged(q, top_k)
                ...
                json.dumps(
                    {
                        ...
                        "candidates_ranked": [] if debug else None,
                    }
                ),
                mimetype="application/json",
                status_code=200,
...
def registry_register(req: func.HttpRequest) -> func.HttpResponse:
    ...
    # Optional convenience: accept wrapped payload {"registry_agent": {...}}
    if isinstance(payload, dict) and "registry_agent" in payload and isinstance(payload["registry_agent"], dict):
        payload = payload["registry_agent"]
...
def registry_get_agent(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        return func.HttpResponse(json.dumps(doc), mimetype="application/json", status_code=200)
    except Exception as e:
...
def registry_patch_agent(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Optional: allow additional metadata-style fields through patch
        for field in ["version", "lifecycle_state", "owner", "environment", "capability_signatures", "metadata"]:
            if field in patch:
                existing[field] = patch[field]
